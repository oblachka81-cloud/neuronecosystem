...
let lastKnownFen = ''; // 🔧 ОПТИМИЗАЦИЯ ПОЛЛИНГА
...
function loadChessPanel() {
  ...
  chessContainer.innerHTML = `
    <div style="max-width:480px;width:100%;margin:0 auto;padding:16px;">
      <button onclick="chessBackToMenu()" style="position:relative;width:100px;height:50px;padding:0;background:none;border:none;cursor:pointer;transition:all 0.2s;overflow:hidden;margin-bottom:20px;" onmouseover="this.style.transform='translateY(-3px)';this.style.filter='brightness(1.15)'" onmouseout="this.style.transform='translateY(0)';this.style.filter='brightness(1)'">
        <img src="/main/btn_duel_back.webp" style="width:100%;height:100%;object-fit:fill;border-radius:16px;display:block;" onerror="this.style.display='none';this.parentElement.innerHTML='<div style=\\'color:#ffcc44;font-weight:700;\\'>← Назад</div>'">
      </button>
      ...
      <div style="background:rgba(10,15,30,0.35);border:1.5px solid rgba(220,220,225,0.5);border-radius:20px;padding:18px 16px;margin-bottom:24px;box-shadow:0 0 20px rgba(220,220,225,0.08), inset 0 1px 0 rgba(255,255,255,0.1);backdrop-filter:blur(8px);">
        <p style="font-size:0.87rem;background:linear-gradient(90deg,#d4d4d8 0%,#ffffff 50%,#d4d4d8 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1.55;margin:0;text-align:center;font-weight:500;letter-spacing:0.3px;">${t.desc}</p>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:20px;">
        <button onclick="chessCreate(100)" style="position:relative;width:100%;height:90px;padding:0;background:none;border:none;cursor:pointer;transition:all 0.2s;overflow:hidden;" onmouseover="this.style.transform='translateY(-3px)';this.style.filter='brightness(1.15)'" onmouseout="this.style.transform='translateY(0)';this.style.filter='brightness(1)'">
          <img src="/main/btn_duel_stake.webp" style="width:100%;height:100%;object-fit:fill;border-radius:16px;display:block;" onerror="this.style.background='#1a2332'">
        ...
        <button onclick="chessCreate(500)" style="position:relative;width:100%;height:90px;padding:0;background:none;border:none;cursor:pointer;transition:all 0.2s;overflow:hidden;" onmouseover="this.style.transform='translateY(-3px)';this.style.filter='brightness(1.2)'" onmouseout="this.style.transform='translateY(0)';this.style.filter='brightness(1)'">
          <img src="/main/btn_duel_stake.webp" style="width:100%;height:100%;object-fit:fill;border-radius:16px;display:block;" onerror="this.style.background='#1a2332'">
          <div style="position:absolute;top:4px;right:6px;background:rgba(0,255,170,0.25);color:#00ffaa;font-size:0.5rem;font-weight:800;padding:2px 5px;border-radius:6px;letter-spacing:0.5px;">HOT</div>
          <div style="position:absolute;inset:0;display:flex;flex-direction:column;justify-content:center;align-items:center;">
            <span style="font-size:0.6rem;color:#ffcc44;font-weight:700;letter-spacing:1px;margin-bottom:2px;text-shadow:0 0 4px rgba(0,0,0,0.8);">СТАВКА</span>
        ...
        <button onclick="chessCreate(1000)" style="position:relative;width:100%;height:90px;padding:0;background:none;border:none;cursor:pointer;transition:all 0.2s;overflow:hidden;" onmouseover="this.style.transform='translateY(-3px)';this.style.filter='brightness(1.15)'" onmouseout="this.style.transform='translateY(0)';this.style.filter='brightness(1)'">
          <img src="/main/btn_duel_stake.webp" style="width:100%;height:100%;object-fit:fill;border-radius:16px;display:block;" onerror="this.style.background='#1a2332'">
          <div style="position:absolute;inset:0;display:flex;flex-direction:column;justify-content:center;align-items:center;">
            <span style="font-size:0.6rem;color:#ffcc44;font-weight:700;letter-spacing:1px;margin-bottom:2px;text-shadow:0 0 4px rgba(0,0,0,0.8);">СТАВКА</span>
...
async function chessCreate(stake) {
  ...
  try {
    const res = await authFetch(`${BASE_URL}/api/chess/create`, {
      ...
      body: JSON.stringify({ stake }) // 🔧 Убрали user_id, бекенд берёт его из заголовка
    });
    ...
    document.getElementById('chessWaitingBlock').innerHTML = `
      <div style="background:rgba(10,15,30,0.8);border:1.5px solid rgba(220,220,225,0.5);border-radius:18px;padding:20px;text-align:center;box-shadow:0 0 20px rgba(220,220,225,0.08), inset 0 1px 0 rgba(255,255,255,0.1);backdrop-filter:blur(8px);">
        ...
        <button onclick="chessCopyLink('${data.inviteLink}', this)" style="position:relative; width:100%; height:54px; padding:0; background:none; border:none; cursor:pointer; transition:all 0.2s; overflow:hidden; margin-bottom:10px; border-radius:12px;">
          <img src="/main/btn_duel_action.webp" style="width:100%;height:100%;object-fit:fill;border-radius:12px;display:block;" onerror="this.style.background='#ffcc44'">
...
function chessShareInvite(link, stake) {
  ...
  if (window.Telegram?.WebApp?.openTelegramLink) { window.Telegram.WebApp.openTelegramLink(shareUrl); return; }
  if (window.Telegram?.WebApp?.openLink) { window.Telegram.WebApp.openLink(shareUrl); return; }
...
function chessCopyLink(link, btn) {
  ...
  const showSuccess = () => {
    if (btn) {
      ...
      btn.querySelector('div').style.color = '#00ffaa';
      setTimeout(() => { 
        btn.innerHTML = originalText; 
        // Восстанавливаем цвет, если картинка не загрузилась, это упрощённый фоллбэк
      }, 2000);
    } else {
...
async function chessCancel(gameIdParam) {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (chessPollInterval) { clearInterval(chessPollInterval); chessPollInterval = null; }
      chessBackToMenu();
      if (typeof loadWelcome === 'function') loadWelcome();
    } else {
...
function chessStartGame(gameId, gameData) {
  chessGameId = gameId;
  lastKnownFen = gameData.fen; // 🔧 Запоминаем начальное состояние для оптимизации
    ...
    for (let col = 0; col < 8; col++) {
      ...
      boardHtml += `
        <div class="chess-cell" data-square="${square}" 
             style="width:12.5%;aspect-ratio:1;display:flex;align-items:center;justify-content:center;font-size:1.8rem;cursor:pointer;background:${isLight ? 'rgba(200,210,230,0.25)' : 'rgba(20,30,50,0.35)'};${chessSelectedSquare === square ? 'box-shadow:inset 0 0 0 3px #00ffaa;' : ''};-webkit-tap-highlight-color:transparent;touch-action:manipulation;user-select:none;">
          ${piece ? `<span style="color:${piece[0] === 'w' ? '#f0f0ff' : '#334466'};text-shadow:0 1px 2px rgba(0,0,0,0.3);">${PIECES[piece]}</span>` : ''}
        </div>`;
    }
  }
...
function chessCellClick(square, gameData) {
  // 🔧 Блокируем взаимодействие, если идёт запрос хода или игра не активна
  ...
  // 🔧 Блокируем клики, если сейчас не ход игрока
  if (!isMyTurn) return;
  ...
  if (!chessSelectedSquare) {
    // 🔧 Разрешаем выделять ТОЛЬКО свои фигуры
  ...
  if (chessSelectedSquare === square) {
    // Снимаем выделение при повторном клике на ту же клетку
    chessSelectedSquare = null;
    chessRenderBoard(gameData.fen, gameData);
    return;
  }

  // 🔧 Если кликнули на другую свою фигуру — просто перевыделяем её
  if (piece && piece[0] === chessMyColor) {
    chessSelectedSquare = square;
    chessRenderBoard(gameData.fen, gameData);
    return;
  ...
  // Попытка хода на пустую клетку или фигуру противника
...
async function chessMakeMove(from, to, gameData) {
  ...
  try {
    const res = await authFetch(`${BASE_URL}/api/chess/move`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      // 🔧 Явно передаём promotion: 'q' (авто-превращение в ферзя)
    ...
    if (stateData.success) {
      if (stateData.game.status === 'finished') {
        ...
        lastKnownFen = stateData.game.fen; // Обновляем известный FEN
    ...
    isMoving = false; // 🔧 Обязательно снимаем блокировку
  }
}
  ...
  chessPollInterval = setInterval(async () => {
    try {
      ...
      if (data.success && data.game.status === 'active') {
        // 🔧 Сравниваем строки FEN напрямую — это мгновенно и на 100% надёжно
...
async function chessResign() {
  ...
  try {
    await authFetch(`${BASE_URL}/api/chess/resign`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ game_id: chessGameId }) // 🔧 Убрали user_id
    });
    chessBackToMenu();
    if (typeof loadWelcome === 'function') loadWelcome();
  } catch (e) {}
...
function chessShowResult(gameData) {
  ...
  if (gameBlock) {
    gameBlock.innerHTML = `
      <div style="background:rgba(10,15,30,0.35);border:1.5px solid rgba(220,220,225,0.5);border-radius:20px;padding:24px 16px;margin-bottom:20px;box-shadow:0 0 30px rgba(220,220,225,0.1), inset 0 1px 0 rgba(255,255,255,0.1);backdrop-filter:blur(8px);text-align:center;">
